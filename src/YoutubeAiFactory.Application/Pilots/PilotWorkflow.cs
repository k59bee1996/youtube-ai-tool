    ...
    PilotGenerationOptions options, TimeProvider timeProvider, ILogger<PilotGenerationJobProcessor> logger)
{
    ...
    public async Task<bool> ProcessNextAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            run = new AiRun("PilotGeneration", payload.ProjectId, resolvedModel.Provider, resolvedModel.Model, PilotGenerationPrompt.Key, PilotGenerationPrompt.Version, now, resolvedModel.Profile.ToString(), jobId: job.Id, workflowStage: "Generation"); store.AddAiRun(run); await store.SaveChangesAsync(cancellationToken);
            ...
            run.CompleteFrom(answer, timeProvider.GetUtcNow()); job.Complete(timeProvider.GetUtcNow()); await store.SaveChangesAsync(cancellationToken); LogCompleted(logger, payload.ProjectId, pilot.Id, context.EligibleIdeaCount, null);
        }
