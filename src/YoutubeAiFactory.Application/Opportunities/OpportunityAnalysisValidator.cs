...
public static class OpportunityAnalysisValidator
{
    public static void Validate(OpportunityAnalysisResult result, OpportunityAnalysisContext context, int maximumCandidates)
    {
        ...
        if (result.Limitations is null) throw new StructuredOutputException("Report limitations are required.");
        ...
        {
            if (item is null || string.IsNullOrWhiteSpace(item.Name) || string.IsNullOrWhiteSpace(item.Description) || string.IsNullOrWhiteSpace(item.Audience) || string.IsNullOrWhiteSpace(item.Topic) || string.IsNullOrWhiteSpace(item.ContentFormat) || string.IsNullOrWhiteSpace(item.Angle) || string.IsNullOrWhiteSpace(item.WhyThisOpportunity))
            ...
            if (item.Risks is null || item.Limitations is null) throw new StructuredOutputException("Opportunity risks and limitations are required.");
