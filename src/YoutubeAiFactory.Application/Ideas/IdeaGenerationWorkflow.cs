...
public sealed class IdeaGenerationJobProcessor(IYoutubeAiFactoryStore store, ILlmProvider provider, IdeaGenerationContextBuilder contextBuilder, IdeaGenerationOptions options, TimeProvider timeProvider, ILogger<IdeaGenerationJobProcessor> logger)
{
    ...
    public async Task<bool> ProcessNextAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var accepted = new List<VideoIdeaCandidateResult>(); var generatedCount = 0; LlmResult<IdeaGenerationResult>? lastAnswer = null;
            for (var replacement = 0; replacement <= options.MaxReplacementAttempts && accepted.Count < options.MinIdeaCount; replacement++)
            {
                ...
                { try { answer = await provider.GenerateStructuredAsync<IdeaGenerationResult>(IdeaGenerationPrompt.Create(context, requestCount, attempt > 0), cancellationToken); break; } catch (StructuredOutputException ex) when (attempt < options.MaxStructuredOutputRetries) { run.RecordRetry(); failure = ex; } catch (Exception ex) { failure = ex; break; } }
                if (answer is null) throw failure ?? new StructuredOutputException("The provider did not return idea output.");
                lastAnswer = answer; run.RecordProvider(answer.Provider, answer.Model); if (answer.Value.Ideas is null || answer.Value.Ideas.Count > options.MaxGeneratedCandidates) throw new StructuredOutputException("The provider returned an invalid number of idea candidates."); generatedCount += answer.Value.Ideas.Count;
