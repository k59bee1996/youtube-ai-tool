...
function PilotPanel({ projectId }: { projectId: string }) {
  const [status, setStatus] = useState<import('./api').PilotStatus | null>(null); const [candidates, setCandidates] = useState<import('./api').PilotCandidate[]>([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState<string | null>(null)
  const active = Boolean(status?.activeJobStatus)
  const load = useCallback(async () => { try { const [loadedStatus, loadedCandidates] = await Promise.all([api.getPilot(projectId), api.getPilotCandidates(projectId)]); setStatus(loadedStatus); setCandidates(loadedCandidates); setError(null) } catch (requestError) { setError(messageFrom(requestError)) } finally { setLoading(false) } }, [projectId])
  ...
  async function generate() { setBusy(true); try { await api.generatePilot(projectId); await load() } catch (requestError) { setError(messageFrom(requestError)) } finally { setBusy(false) } }
  async function approve() { if (!status?.latestPilot) return; setBusy(true); try { await api.approvePilot(projectId, status.latestPilot.id); await load() } catch (requestError) { setError(messageFrom(requestError)) } finally { setBusy(false) } }
  async function move(sequence: number, direction: 'up' | 'down') { if (!status?.latestPilot) return; setBusy(true); try { await api.movePilotSlot(projectId, status.latestPilot.id, sequence, direction); await load() } catch (requestError) { setError(messageFrom(requestError)) } finally { setBusy(false) } }
  async function replace(sequence: number, videoIdeaId: string) { if (!status?.latestPilot || !videoIdeaId) return; setBusy(true); try { await api.replacePilotSlot(projectId, status.latestPilot.id, sequence, videoIdeaId); await load() } catch (requestError) { setError(messageFrom(requestError)) } finally { setBusy(false) } }
  const pilot = status?.latestPilot
