...
function PilotPanel({ projectId }: { projectId: string }) {
  ...
  return <section className="analysis-section panel pilot-panel"><div className="section-heading"><div><span className="eyebrow">Validation</span><h2>12-Video Pilot</h2></div>{pilot && <span>Version {pilot.version} · {pilot.status}</span>}</div>
    {loading ? <p className="empty-state">Loading pilot…</p> : error ? <p className="error-copy" role="alert">{error}</p> : active ? <div className="analysis-state"><strong>Generating 12-video pilot…</strong><span>Status: {status?.activeJobStatus}</span></div> : !pilot ? <div className="analysis-state"><p>{status?.eligibleIdeaCount ?? 0} approved ideas available. At least 12 are required to create a complete pilot.</p><button className="primary-button" type="button" onClick={() => void generate()} disabled={busy || (status?.eligibleIdeaCount ?? 0) < 12}>{busy ? 'Queuing pilot…' : 'Generate 12-Video Pilot'}</button></div> : <PilotView pilot={pilot} candidates={candidates} onApprove={approve} onMove={move} onReplace={replace} busy={busy} />}
  </section>
}
