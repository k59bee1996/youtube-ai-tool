...
import { api, type CompetitorDetails, type CompetitorSummary, type CreateProjectRequest, type Project } from "../../api";
import { CompetitorDetails as CompetitorDetailsView } from "../../features/competitors/CompetitorDetails";
import { OpportunityPanel } from "../../features/opportunities/OpportunityPanel";
import { PilotPanel } from "../../features/pilots/PilotPanel";
...
export function WorkspacePage() {
  ...
  async function openProject(selected: Project) {
    ...
    try {
      const loadedCompetitors = await api.listCompetitors(
        selected.id,
        controller.signal,
      );
  ...
  async function addCompetitor(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const collected = await api.addCompetitor(
        projectId,
        youtubeUrl,
        controller.signal,
      );
  ...
  function isCurrentDetail(
    ...
    return (
      isCurrentWorkspace(workspace, projectId) &&
      (request === null || detailRequest.current === request)
    );
  }

  function isCurrentCollection(
    workspace: number,
    projectId: string,
    request: number,
  ) {
    ...
    <div className="home-grid">
      <section className="panel">
        ...
        <p className="section-copy">
          Define the market and audience for this research workspace.
        </p>
        <form className="form-grid" onSubmit={onCreate}>
          <label>
            ...
            <input
              ...
              required
              maxLength={200}
              placeholder="Creator education research"
            />
          </label>
          <label>
            Market
      ...
      <section className="panel">
        ...
        {projects.length === 0 ? (
          <p className="empty-state">
            No projects yet. Create the first one to begin.
          </p>
        ) : (
          <div className="project-list">
            {projects.map((item) => (
              ...
              >
                ...
                <span>
                  {item.marketName} · {item.targetLanguage} ·{" "}
                  {item.targetGeography}
                </span>
              </button>
            ))}
          </div>
  ...
  return (
    <>
      <section className="project-context">
        <div>
          <span>Market</span>
          <strong>{project.marketName}</strong>
        </div>
        <div>
          <span>Audience</span>
          <strong>{project.audienceDescription}</strong>
        </div>
        <div>
          <span>Target</span>
          <strong>
            {project.targetLanguage} · {project.targetGeography}
          </strong>
        </div>
      </section>
      ...
      <section className="panel competitor-panel">
        <div className="section-heading">
          <div>
            <h2>Competitors</h2>
            <p className="section-copy">
              Collect a channel and its configured recent upload window.
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <CompetitorDetailsView competitor={competitor} />
      ) : (
        <p className="empty-state">
          Add a YouTube channel to begin competitor research.
        </p>
      )}
      <section className="panel competitor-panel">
        <div className="section-heading">
          <div>
            ...
            <h2>Opportunities</h2>
            <p className="section-copy">
              Synthesize completed competitor analyses into ranked,
          ...
          >
            {opportunitiesOpen ? "Hide opportunities" : "Open opportunities"}
          </button>
        </div>
      </section>
      ...
      <section className="panel competitor-panel">
        <div className="section-heading">
          <div>
            ...
            <p className="section-copy">
              Turn approved ideas into a deliberate topic, packaging, and
              storytelling learning plan.
            </p>
          </div>
          <button
            className="primary-button"
            type="button"
            onClick={() => setPilotOpen((open) => !open)}
          >
            {pilotOpen ? "Hide pilot" : "Open pilot"}
          </button>
        </div>
      </section>
...
function readSelectedProjectId() {
  try { return localStorage.getItem(selectedProjectKey); } catch { return null; }
}

function tryPersistSelectedProject(projectId: string | null) {
  try {
    if (projectId === null) localStorage.removeItem(selectedProjectKey);
    else localStorage.setItem(selectedProjectKey, projectId);
