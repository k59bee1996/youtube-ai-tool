...
import { api, type ProductionAsset, type ProductionHistoryItem, type ProductionPackage, type ProductionScene, type ProductionStatus, type ProductionUpdate } from "../../services/api"
...
export function ProductionWorkspace({ projectId, videoProjectId, revisionKey, onWorkflowStatusChange }: {
  ...
  return <section className="analysis-block production-workspace">
    ...
    {current ? <PackageView value={current} editing={editing} isLatest={viewed?.id === status?.latestPackage?.id} busy={busy} canGenerate={Boolean(status?.canGenerate)}
      ...
      onExport={exportJson} /> : null}
  </section>
}

function PackageView({ value, editing, isLatest, busy, canGenerate, onChange, onRegenerate, onEdit, onCancel, onSave, onValidate, onApprove, onExport }: {
  ...
  return <div className="analysis-report">
    ...
    {value.groundingIssues.map((issue, index) => <p className={issue.severity === "Error" ? "error-copy" : "stale-note"} key={`${issue.sceneSequence}-${index}`}><strong>{issue.severity}: {issue.issueType}</strong> · Scene {issue.sceneSequence}: {issue.explanation}</p>)}
    <DirectionFields value={value} editing={editing} onChange={update} />
    <section className="analysis-block"><h4>Scenes</h4>{value.scenes.map(scene => <SceneCard key={scene.id} scene={scene} editing={editing} assets={assets} onChange={updateScene} />)}</section>
    <section className="analysis-block"><h4>Reusable assets</h4>{value.assetRequirements.map(asset => <AssetCard key={asset.id} asset={asset} editing={editing} onChange={updateAsset} />)}</section>
...
function SceneCard({ scene, editing, assets, onChange }: { scene: ProductionScene; editing: boolean; assets: Map<string, ProductionAsset>; onChange: (scene: ProductionScene) => void }) {
  ...
  return <article className="script-section-card"><div className="section-heading"><div><span className="eyebrow">Scene {scene.sequence} · {scene.purpose}</span><strong>{formatDuration(scene.estimatedDurationSeconds)} · {scene.complexity}</strong></div></div>
    ...
    <Editable label="Narration summary" value={scene.narrationSummary} editing={editing} onChange={narrationSummary => set({ narrationSummary })} />
    <Editable label="Visual strategy" value={scene.visualStrategy} editing={editing} onChange={visualStrategy => set({ visualStrategy })} />
    ...
    <details open><summary>Shots ({scene.shots.length})</summary>{scene.shots.map(shot => <div className="script-block" key={shot.id}><strong>Shot {shot.sequence} · {shot.shotType} · {formatDuration(shot.estimatedDurationSeconds)}</strong><Editable label="Visual" value={shot.visualDescription} editing={editing} onChange={visualDescription => set({ shots: scene.shots.map(item => item.id === shot.id ? { ...item, visualDescription } : item) })} /><Editable label="Composition" value={shot.composition} editing={editing} onChange={composition => set({ shots: scene.shots.map(item => item.id === shot.id ? { ...item, composition } : item) })} /><Editable label="Motion" value={shot.motionSuggestion} editing={editing} onChange={motionSuggestion => set({ shots: scene.shots.map(item => item.id === shot.id ? { ...item, motionSuggestion } : item) })} /><p>{shot.factualityMode}{shot.assetRequirementId ? ` · Asset ${assets.get(shot.assetRequirementId)?.assetKey ?? shot.assetRequirementId}` : ""}</p><ClaimList claims={shot.claims} /></div>)}</details>
...
function AssetCard({ asset, editing, onChange }: { asset: ProductionAsset; editing: boolean; onChange: (asset: ProductionAsset) => void }) {
  const set = (fields: Partial<ProductionAsset>) => onChange({ ...asset, ...fields })
  return <article className="pilot-card"><strong>{asset.assetKey} · {asset.assetType}</strong><p>{asset.acquisitionMode} · {asset.factualityMode} · {asset.complexity}{asset.rightsVerificationRequired ? " · rights check required" : ""}</p><Editable label="Creative brief" value={asset.creativeBrief} editing={editing} onChange={creativeBrief => set({ creativeBrief })} /><Editable label="Generation prompt" value={asset.generationPrompt} editing={editing} onChange={generationPrompt => set({ generationPrompt })} /><Editable label="Source search brief" value={asset.sourceSearchBrief} editing={editing} onChange={sourceSearchBrief => set({ sourceSearchBrief })} /><ClaimList claims={asset.claims} /></article>
}
