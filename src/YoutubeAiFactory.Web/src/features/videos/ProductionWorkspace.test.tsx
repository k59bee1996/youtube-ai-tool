...
test("Vietnamese reading aid changes only localizable explanations", async () => {
  const value=readyPackage()
  vi.stubGlobal("fetch",vi.fn((input:RequestInfo|URL,init?:RequestInit)=>{const url=String(input);const method=init?.method??"GET";if(url.endsWith("/production-package/latest"))return json(status({latestPackage:value,canGenerate:true,blockReason:null}));if(url.endsWith("/production-packages")&&method==="GET")return json([history(value)]);if(url.endsWith(`/production-packages/${value.id}/localizations/vi`)&&method==="GET")return json({content:{canonicalContentFingerprint:"a".repeat(64),warnings:[],groundingIssues:[],scenes:[{sceneId:"scene-1",narrationSummary:"Giải thích nhân sự",visualStrategy:"Sơ đồ",shots:[{shotId:"shot-1",visualDescription:"Sơ đồ nhân sự lâu đài có nhãn"}]}],assets:[{assetId:"asset-1",creativeBrief:"Sơ đồ có nhãn",sourceSearchBrief:""}]},activeJob:null,latestJob:null});throw new Error(`Unexpected ${method} ${url}`)}))
  render(<ProductionWorkspace projectId="project-1" videoProjectId="video-1" revisionKey="r1" onWorkflowStatusChange={vi.fn()} />)

  fireEvent.click(await screen.findByRole("button",{name:"VI"}))

  expect(await screen.findByText("Sơ đồ nhân sự lâu đài có nhãn")).toBeVisible()
  expect(screen.getByText("Castle ownership required recurring staffing obligations.")).toBeVisible()
  expect(screen.getByText("Canonical generation prompt")).toBeVisible()
})
