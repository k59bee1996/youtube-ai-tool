...
import { api, type ResearchLocalizationStatus, type ResearchReport, type ResearchStatus, type VideoProject, type VideoProjectListItem } from "../../services/api"
...
export function VideoProjectWorkspace({ project, onBack }: { project: VideoProject; onBack: () => void }) {
  ...
  return <section className="analysis-section panel">
    ...
    <div className="confidence-grid"><div><span>Planning</span><strong>Complete</strong></div><div><span>Research</span><strong>{research?.activeJob ? research.activeJob.status : research?.latestReport ? "Complete" : "Not started"}</strong></div><div><span>Outline</span><strong>Locked</strong></div><div><span>Script</span><strong>Locked</strong></div><div><span>Production</span><strong>Locked</strong></div></div>
    ...
    <section className="analysis-block"><h3>Research</h3>{researchError ? <p className="error-copy" role="alert">{researchError}</p> : null}{research?.activeJob ? <><p>Research is running: {research.activeJob.status}. It plans questions, retrieves public sources, extracts evidence, checks conflicts, and builds a report.</p><button className="quiet-button" type="button" onClick={() => void loadResearch()}>Refresh research status</button></> : research?.latestReport ? <ResearchReportView key={research.latestReport.id} projectId={current.projectId} videoProjectId={current.id} report={research.latestReport} onRerun={runResearch} busy={researchBusy} /> : <><p className="section-copy">No research has been created for this video. Research gathers external sources, extracts evidence, and builds a fact-backed report.</p>{research?.latestJob?.failureReason ? <p className="error-copy" role="alert">Research failed: {research.latestJob.failureReason}</p> : null}<button className="primary-button" type="button" disabled={researchBusy} onClick={() => void runResearch()}>{researchBusy ? "Queuing research..." : research?.latestJob?.status === "Failed" ? "Retry Research" : "Run Research"}</button></>}</section>
  </section>
}

function ResearchReportView({ projectId, videoProjectId, report, onRerun, busy }: { projectId: string; videoProjectId: string; report: ResearchReport; onRerun: () => Promise<void>; busy: boolean }) {
