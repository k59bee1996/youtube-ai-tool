...
export function VideoProjectWorkspace({ project, onBack }: { project: VideoProject; onBack: () => void }) {
  ...
  return <section className="analysis-section panel">
    ...
    <div className="confidence-grid"><div><span>Planning</span><strong>Complete</strong></div><div><span>Research</span><strong>{research?.activeJob ? research.activeJob.status : refreshFailure ? "Refresh failed" : research?.latestReport ? "Complete" : "Not started"}</strong></div><div><span>Outline</span><strong>{current.status.startsWith("Outline") ? current.status.replace("Outline", "") || "Generating" : current.status.startsWith("Script") ? "Approved" : "Locked"}</strong></div><div><span>Script</span><strong>{current.status.startsWith("Script") ? current.status.replace("Script", "") || "Generating" : "Locked"}</strong></div><div><span>Production</span><strong>Locked</strong></div></div>
    <nav className="workspace-tabs" role="tablist" aria-label="Video Project workflow">{(["Overview", "Research", "Outline", "Script", "Production"] as WorkspaceTab[]).map(tab => <button className={activeTab === tab ? "workspace-tab active" : "workspace-tab"} type="button" role="tab" aria-selected={activeTab === tab} disabled={tab === "Production"} key={tab} onClick={() => setActiveTab(tab)}>{tab}{tab === "Production" ? " (locked)" : ""}</button>)}</nav>
