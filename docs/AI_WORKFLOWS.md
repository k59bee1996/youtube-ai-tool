...
## Production package generation and grounding

Input is a current, Approved, grounding-passed Script plus its exact Outline/Research lineage, project settings, viewer promise, content format, and Pilot experiment. Context contains bounded structured ScriptBlocks, Claims, evidence excerpts/source locators, and conflicts; no raw page body, search, fetch, media-generation provider, or new Research is used.

Model routing is explicit and provider-neutral: generation uses `ProductionPackageGeneration` (Reasoning), grounding audit uses `ProductionGroundingAudit` (Reasoning), correction uses `ProductionPackageCorrection` (Reasoning), and only malformed structured-output repair uses Fast. AiRun records persist workflow, model profile, resolved provider/model, prompt key/version, token counts when available, latency, retry count, and failures.

Application validation—not the model—owns exact ScriptBlock coverage/order, configured limits, enum validity, claim reachability, rights flags, generated-versus-archival semantics, factual overlay/chart/map requirements, and all timing math. Scene seconds are allocated from mapped Script word counts and shot seconds from relative weights; both sums are exact and package duration always equals the approved Script estimate.

The independent audit flags unsupported visual facts/text/numbers/dates/maps/charts, visual-Script contradiction, conflict misrepresentation, false archival impression, and claim mismatch. A failed initial candidate may receive one bounded correction and re-audit. User edits never invoke correction or generation: they mark grounding Pending, and `:validate` performs audit only.

`production-package-localization:v1` uses the shared lazy Fast localization queue and the `(artifact_type, artifact_id, artifact_version, locale)` cache. It translates only reader-facing warnings, grounding explanations, scene narration summaries/visual strategies, shot visual descriptions, and asset creative/source-search briefs. Validation requires the canonical fingerprint plus exact warning/issue indexes and Scene/Shot/Asset IDs, order, count, and empty-field topology. Narration, generation prompts, on-screen copy, composition/motion instructions, IDs, claims, URLs, enums, statuses, versions, timing, classifications, lineage, and JSON export remain canonical. A translation miss coalesces to one job; failure leaves English immediately available.

## Phase 12 observability and cost accounting

The dashboard is a deterministic read workflow over persisted `Job` and `AiRun` records. A logical workflow Job may contain several provider requests (generation, audit, correction, repair, or paid retries), and each request is retained as its own AiRun when recorded. Project strategy requests remain shared project cost; Research, Outline, Script, and Production Package requests with a VideoProject association remain direct cost. Provider-reported cost, explicitly price-calculated estimates, and unavailable cost are distinct. Unknown values are visible in coverage counts and are never treated as zero. Dashboard reads do not invoke an LLM, enqueue work, translate artifacts, or alter VideoProject states.
