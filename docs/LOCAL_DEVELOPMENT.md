...
Phase 11 production-package generation and edited-package validation also require the Worker and `AI__ApiKey`. Configure bounds under `ProductionPackage`. The workflow calls only `ILlmProvider`; it has no image/video/TTS SDK, storage write, or renderer dependency. Apply `AddProductionPackages` through normal EF Core migration startup/update before using the endpoints.
