...
- `POST /api/projects/{projectId}/pilots:generate` queues a project-level Pilot from approved ideas and returns `202`. A matching queued/running job is reused.
...
- `POST /api/projects/{projectId}/pilots/{pilotId}:approve` approves a draft Pilot unless it requires review because a source idea is no longer approved.
...
At least 12 approved ideas are required. The API does not generate ideas automatically, ingest analytics, create VideoProjects, or start production.
