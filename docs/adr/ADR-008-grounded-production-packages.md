...
The relational shape is larger than a single JSON payload and requires explicit mapping code, but it makes ordering, uniqueness, ownership, and evidence integrity enforceable. Original relative-duration weights are retained beside allocated seconds, and package optimistic concurrency prevents stale approval after a racing edit. A separate localization overlay may translate only selected reader-facing explanations; canonical narration, generation prompts, on-screen copy, instructions, identities, timing, and export never change.
